...
export default async function getAssociatedProjects(
  ProjectModel: ReturnModelType<typeof Project>,
  input: {userId: string; role: string}
) {
  //   let query;
  //   if (input.role === 'Agent') {
  //     query = {associatedAgent: input.userId};
  //   } else {
  //   }
  // const projects = await ProjectModel.find(
  //   input.role === UserRole.CLIENT
  //     ? {associatedClient: input.userId}
  //     : {associatedDesigner: input.userId}
  // ).populate(['associatedClient', 'associatedDesigner']);
